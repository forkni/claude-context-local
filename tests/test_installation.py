...
class TestCUDADetection:
    ...
    def test_cuda_version_detection_121(self):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            assert cuda_info['pytorch_index'] == 'https://download.pytorch.org/whl/cu121'
    ...
    def test_cuda_version_detection_118(self):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            assert cuda_info['pytorch_index'] == 'https://download.pytorch.org/whl/cu118'
    ...
    def _parse_cuda_info(self, nvidia_smi_output):
        ...
        # Extract CUDA version (simplified simulation)
        cuda_version = "12.1"  # From mock data
    ...
    def _detect_cuda_availability(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return {'available': False, 'version': None, 'pytorch_index': 'https://pypi.org/simple'}
        except (subprocess.TimeoutExpired, FileNotFoundError):
            return {'available': False, 'version': None, 'pytorch_index': 'https://pypi.org/simple'}
...
class TestInstallationModes:
    ...
    def test_auto_install_with_cuda(self):
        ...
        assert install_config['pytorch_index'] == 'https://download.pytorch.org/whl/cu121'
    ...
    def test_manual_cuda_selection(self):
        ...
        assert install_config['mode'] == 'cuda'
        assert install_config['pytorch_index'] == 'https://download.pytorch.org/whl/cu118'
...
class TestDependencyInstallation:
    ...
    def test_pytorch_installation_cuda(self):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            result = self._install_pytorch_cuda("https://download.pytorch.org/whl/cu121")
            assert result['success'] is True
    ...
    def _create_virtual_environment(self, venv_path):
        ...
        try:
            ...
            return {'success': True, 'python_executable': str(venv_path / "Scripts" / "python.exe")}
    ...
    def _install_hybrid_search_deps(self):
        ...
        return {
            'success': True,
            'packages_installed': ['rank-bm25', 'nltk']
        }
    ...
    def _download_nltk_data(self):
        ...
        return {
            'success': True,
            'data_downloaded': ['stopwords', 'punkt']
        }
...
class TestInstallationVerification:
    ...
    def _verify_mcp_server(self):
        """Verify MCP server."""
        try:
            result = subprocess.run(['python', '-m', 'mcp_server.server', '--help'],
                                  capture_output=True, text=True, timeout=10)
...
class TestErrorHandling:
    ...
    def test_insufficient_disk_space(self):
        ...
        with patch('shutil.disk_usage', return_value=(1000, 100, 100)):  # Very low space
    ...
    def _validate_cuda_driver_compatibility(self, cuda_info):
        ...
        return {
            ...
            'issue': 'driver version mismatch' if 'Version: 12.1, Driver Version: 11.8' in cuda_info else None
        }
