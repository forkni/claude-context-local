...
ERROR_SCENARIOS = {
    "python_not_found": {
        ...
        "error": FileNotFoundError("'python' is not recognized as an internal or external command"),
        "exit_code": 1,
    ...
    "nvidia_smi_not_found": {
        "command": "nvidia-smi",
        "error": FileNotFoundError("'nvidia-smi' is not recognized as an internal or external command"),
        "exit_code": 1,
...
class MockInstallationEnvironment:
    ...
    def simulate_python_check(self) -> Dict:
        ...
        if not self.has_python:
            return {
                'success': False,
                'error': ERROR_SCENARIOS['python_not_found']
            }
    ...
    def simulate_venv_creation(self) -> Dict:
        ...
        if self.disk_space_gb < 1:
            return {
                'success': False,
                'error': ERROR_SCENARIOS['insufficient_space']
            }
    ...
    def simulate_uv_installation(self) -> Dict:
        ...
        if not self.has_network:
            return {
                'success': False,
                'error': ERROR_SCENARIOS['network_error']
            }
...
def get_mock_environment(env_name: str) -> MockInstallationEnvironment:
    ...
    if env_name not in TEST_ENVIRONMENTS:
        raise ValueError(f"Unknown environment: {env_name}. Available: {list(TEST_ENVIRONMENTS.keys())}")
