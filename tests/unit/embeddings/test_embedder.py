...
class TestContextExtraction:
    """Test create_embedding_content's delegation to the document composer
    (v0.8.0+ context features, config-fetch half only).

    The composer's own behaviour (import/class-signature extraction, its
    caches) is tested directly in
    tests/unit/embeddings/test_document_composer.py, which never imports
    CodeEmbedder — that import-absence is the model-freeness proof.
    """
...
class TestEmbedChunksContentHashCache:
    ...
    def test_changed_import_block_changes_key_for_unchanged_chunk(
        ...
        embedder._document_composer.clear_caches()
