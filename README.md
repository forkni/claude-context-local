...
- **29 File Extensions**: Python, JS, TS, Go, Rust, C/C++/CUDA, C#, GLSL with AST/tree-sitter chunking
...
| Rust | `.rs` | Tree-sitter |
| C | `.c` | Tree-sitter |
| C++ / CUDA | `.cpp`, `.cc`, `.cxx`, `.c++`, `.h`, `.hpp`, `.hh`, `.hxx`, `.inl`, `.ipp`, `.tpp`, `.cu`, `.cuh` | Tree-sitter (C++) |
| C# | `.cs` | Tree-sitter |
| GLSL | `.glsl`, `.frag`, `.vert`, `.comp`, `.geom`, `.tesc`, `.tese`, `.glslinc` | Tree-sitter |

**Total**: 29 file extensions across 9 programming languages (`.cu`/`.cuh` use the C++ grammar)
