...
class GLSLChunker(LanguageChunker):
    ...
    def extract_metadata(self, node: Any, source: bytes) -> Dict[str, Any]:
        ...
        # Extract variable declarations (uniforms, varying, attributes)
            ...
            for child in node.children:
                if child.type in ['uniform', 'varying', 'attribute', 'in', 'out', 'const']:
        ...
        if any(keyword in node_text for keyword in ['gl_position', 'gl_fragcoord', 'gl_fragcolor']):
            metadata['has_builtin_vars'] = True

        # Check for texture operations
        if any(keyword in node_text for keyword in ['texture', 'texturelod', 'texture2d', 'sampler']):
            metadata['has_texture_ops'] = True

        # Check for mathematical operations
        if any(keyword in node_text for keyword in ['dot', 'cross', 'normalize', 'length', 'mix', 'smoothstep']):
