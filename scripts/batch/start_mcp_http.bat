...
rem Name:     start_mcp_http.bat
rem Purpose:  Start MCP Server with StreamableHTTP transport on port 8765
rem Note:     Avoids stdio transport bugs in Claude Code 2.0.22+
rem Author:   claude-context-local project
rem Revision: 2026-05-25 - Migrated from SSE to StreamableHTTP transport

echo ============================================================
echo MCP Server HTTP Mode
echo ============================================================
echo Server will run on: http://localhost:8765/mcp
...
"%~dp0..\..\.venv\Scripts\python.exe" -m mcp_server.server --transport http --host localhost --port 8765
