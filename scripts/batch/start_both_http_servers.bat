...
echo ================================================================
echo Dual HTTP Server Setup
echo ================================================================
echo.
echo Launching two MCP StreamableHTTP server instances:
echo   - VSCode Extension: http://localhost:8765/mcp
echo   - Native CLI:       http://localhost:8766/mcp
echo.
...
start "MCP HTTP - VSCode (8765)" cmd /k ""%~dp0..\..\scripts\batch\start_mcp_http.bat""

timeout /t 3 >nul

echo [2/2] Starting CLI server (Port 8766)...
REM Use absolute path to avoid quote parsing bugs with delayed expansion
start "MCP HTTP - CLI (8766)" cmd /k ""%~dp0..\..\scripts\batch\start_mcp_http_cli.bat""
...
if !SERVER_8765_RUNNING! equ 1 (
    echo [OK] VSCode Server: http://localhost:8765/mcp - RUNNING
) else (
    echo [ERROR] VSCode Server: Port 8765 - NOT RUNNING
)

if !SERVER_8766_RUNNING! equ 1 (
    echo [OK] CLI Server:    http://localhost:8766/mcp - RUNNING
) else (
