...
REM Merges ONLY files in docs/ directory, excluding development-only docs
...
if !NON_DOCS_COUNT! GTR 0 (
    ...
    echo.
    git diff --name-only main development | findstr /v "^docs/"
    echo.
...
set BACKUP_TAG=pre-docs-merge-%datetime:~0,8%-%datetime:~8,6%
git tag %BACKUP_TAG%
if %ERRORLEVEL% EQU 0 (
...
git commit -m "docs: Sync documentation from development

- Updated docs/ directory from development branch
- Docs-only update (no code changes)
