...
REM For hotfixes and urgent changes that can't wait for full merge
...
git show !COMMIT_HASH! --name-only --format="" | findstr "^tests/" >nul
if %ERRORLEVEL% EQU 0 set HAS_EXCLUDED_FILES=1

git show !COMMIT_HASH! --name-only --format="" | findstr "^docs/TESTING_GUIDE.md" >nul
if %ERRORLEVEL% EQU 0 set HAS_EXCLUDED_FILES=1

git show !COMMIT_HASH! --name-only --format="" | findstr "pytest.ini" >nul
if %ERRORLEVEL% EQU 0 set HAS_EXCLUDED_FILES=1
...
if !HAS_EXCLUDED_FILES! EQU 1 (
    ...
    git show !COMMIT_HASH! --name-only --format="" | findstr "^tests/ ^docs/TESTING_GUIDE.md pytest.ini"
    echo.
...
set BACKUP_TAG=pre-cherry-pick-%datetime:~0,8%-%datetime:~8,6%
git tag %BACKUP_TAG%
if %ERRORLEVEL% EQU 0 (
