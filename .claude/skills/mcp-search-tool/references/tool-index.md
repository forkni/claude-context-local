...
|------|------|---------|----------------------|
| `code-search:list_projects` | Core | Show all indexed projects, each model's chunk count, and live freshness via `last_indexed_at` (Merkle `last_snapshot`, advances on every re-index). **Not** `created_at` — that's frozen at first-index registration and never updates. | — |
| `code-search:switch_project` | Core | Switch active project | — |
| `code-search:get_index_status` | Core | Check index health and staleness (`last_indexed_time`) for the **active project only** — use `list_projects`' `last_indexed_at` to check another project's freshness without switching | — |
