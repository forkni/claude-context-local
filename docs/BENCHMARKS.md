...
### Results (2026-09-05b, hybrid-only, k=10, deterministic, ADR-0063 re-pin)

Provenance: `evaluation/CANON_20260905B_ADR0063_REBASELINE.md`, `scripts/benchmark/run_sscg_benchmark.py --project-path .`, default config (`bm25_weight=0.35`, `dense_weight=0.65`, `query_expansion.enabled=False`, `intent.enabled=true` matching the shipped default), PYTHONHASHSEED=0 deterministic harness (ADR-0021). Re-pin after `721ccde` (decorated methods get `parent_chunk_id`) and `135007c`/ADR-0063 (decorated classes become container nodes, so their methods are chunked at all) — both search-substrate changes per this project's standing re-pin rule. Full non-incremental reindex (234 files / 2,945 chunks, +74 vs the prior pin; 30,286 edges including 1,095 `contains` edges, +201; resolver mix on `calls` edges `lsp 1925 / libcst 752 / pyan 551`), `audit_golden_dataset.py` clean on both datasets before capture. 63q determinism reconfirmed bit-identical (r2 vs r1, 0/63 `retrieved`-list diffs). Only **hybrid** (the default mode) has been measured at this generation — no per-mode A/B has been rerun since 2026-06-08 (historical table below).

| Dataset | Queries | MRR | Recall@5 | Recall@10 | NDCG@5 | pool_hit_rate |
|---|---|---|---|---|---|---|
| Canonical (`golden_dataset.json`, A–F excl. D) | 63 | **0.8164** | 0.6390 | 0.7612 | 0.6678 | 1.0000 |
| Expanded (`golden_dataset_expanded.json`, non-D, 133 queries) | 133 | **0.6286** | 0.6002 | 0.7175 | 0.5787 | — |
| F-via-similar (anchor-chunk view, whole-63q aggregate) | 63 | **0.8671** | 0.6505 | 0.7730 | 0.6876 | — |

Deltas vs the 2026-09-05 pin (0.8234/0.6223/0.8697) are −0.0070 / +0.0063 / −0.0026, all well inside the pre-registered ±0.02 drift band (recall@20 deltas +0.0010 / −0.0083 / +0.0040, both ≥ the −0.02 floor) — gate PASSED. Read as noise around a flat baseline: the +74 new chunks and +201 new `contains` edges from ADR-0063 moved no metric anywhere near a threshold crossing (see `evaluation/CANON_20260905B_ADR0063_REBASELINE.md` Scope note).
