...
### Results (2026-09-05, hybrid-only, k=10, deterministic, contains-edge re-pin)

Provenance: `evaluation/CANON_20260905_REBASELINE.md`, `scripts/benchmark/run_sscg_benchmark.py --project-path .`, default config (`bm25_weight=0.35`, `dense_weight=0.65`, `query_expansion.enabled=False`, `intent.enabled=true` matching the shipped default), PYTHONHASHSEED=0 deterministic harness (ADR-0021). Re-pin after a 16-commit search-path burst (`5d50708`…`abeef6f`), most notably `abeef6f` emitting class→method `contains` edges from `parent_chunk_id` — a search-substrate change per this project's standing re-pin rule. Full non-incremental reindex (234 files / 2,871 chunks; 29,664 edges including 894 new `contains` edges — one per method chunk; resolver mix on `calls` edges `lsp 1911 / libcst 745 / pyan 558`), `audit_golden_dataset.py` clean on both datasets before capture. 63q determinism reconfirmed bit-identical (r2 vs r1, 0/63 `retrieved`-list diffs). Only **hybrid** (the default mode) has been measured at this generation — no per-mode A/B has been rerun since 2026-06-08 (historical table below).

| Dataset | Queries | MRR | Recall@5 | Recall@10 | NDCG@5 | pool_hit_rate |
|---|---|---|---|---|---|---|
| Canonical (`golden_dataset.json`, A–F excl. D) | 63 | **0.8234** | 0.6564 | 0.7538 | 0.6842 | 1.0000 |
| Expanded (`golden_dataset_expanded.json`, non-D, 133 queries) | 133 | **0.6223** | 0.6166 | 0.7296 | 0.5913 | — |
| F-via-similar (anchor-chunk view, whole-63q aggregate) | 63 | **0.8697** | 0.6610 | 0.7558 | 0.6961 | — |

Deltas vs the 2026-09-03 pin (0.8429/0.6332/0.8856) are −0.0195 / −0.0109 / −0.0159, all inside the pre-registered ±0.02 drift band (recall@20 deltas −0.0097 / +0.0023 / −0.0013, both ≥ the −0.02 floor) — gate PASSED, read as substrate drift across the whole 16-commit burst, **not** an isolated measurement of `contains` alone (see `evaluation/CANON_20260905_REBASELINE.md` Scope note). The 63q delta is the closest to the drift-band boundary of any re-pin recorded in this project's history; worth watching on the next re-pin, not a trip.
