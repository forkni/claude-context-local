...
**Honest reading**: treatment vs control point estimates were mixed and small — 63q and F-via-similar MRR slightly negative (−0.0086, −0.0077), 133q MRR slightly positive (+0.0011); every CI spans 0 by a wide margin. One query (H035) flipped HIT→MISS on the 133q leg but its CI already spans 0. Adoption rests on the gate criteria (statistical indistinguishability) plus the correctness case — the truncation defect is real and the golden set was never positioned to target the 25 affected preamble runs either way. See `evaluation/CANON_20260923_PREAMBLE_PACKING.md` for the full control-vs-treatment table and all three gates' CIs.

| Dataset | Queries | MRR | Recall@5 | Recall@10 | Recall@20 | NDCG@5 | pool_hit_rate |
|---|---|---|---|---|---|---|---|
| Canonical (`golden_dataset.json`, A–F excl. D) | 63 | **0.8177** | — | 0.7400 | 0.8144 | — | 1.0000 |
| Expanded (`golden_dataset_expanded.json`, non-D, 133 queries) | 133 | **0.6527** | — | 0.6908 | 0.7774 | — | 0.9173 |
| F-via-similar (anchor-chunk view, whole-63q aggregate) | 63 | **0.8668** | — | 0.7454 | 0.7888 | — | 1.0000 |

Both caveats from the 2026-09-20 section below still apply unchanged: (1) these numbers require the non-default `search_config.json` this machine ships (`codefuse-ai/F2LLM-v2-0.6B` + `jinaai/jina-reranker-v3`); (2) measured with the intent layer pinned off, not the live `default_intent: "HYBRID"` serving config.

### Results (2026-09-23, hybrid-only, k=10, deterministic, `fill_budget` A/B adoption re-pin — superseded by the `split_oversized_preamble` pin above)

Provenance: `evaluation/CANON_20260923_REBASELINE.md`, `scripts/benchmark/run_sscg_benchmark.py --project-path .`, default config (`bm25_weight=0.35`, `dense_weight=0.65`, `query_expansion.enabled=False`, `intent.enabled=true` matching the shipped default), PYTHONHASHSEED=0 deterministic harness (ADR-0021), `CLAUDE_AUTO_REINDEX=0` exported for every leg. `/diagnose` on a 2026-09-22 reindex note found `EmbeddingDocumentComposer.compose()`'s legacy 20-head/10-tail *line* caps bind long before the 6,000-char budget does for chunks with short average line length — 307/307 sampled oversized repo chunks (2.7% of the corpus) composed to well under 0.8x their remaining budget (p50 0.22x). Fixed behind `EmbeddingConfig.body_truncation` (`"head_tail_lines"` legacy / `"fill_budget"` new — fills head/tail by character budget, no line caps), adopted as the default after a pre-registered A/B (control = `head_tail_lines` at clean HEAD `4a36137a`, treatment = `fill_budget`, identical substrate otherwise — 238 files / 3,071 chunks, resolver mix `lsp 2041 / chunker 1133 / libcst 775 / pyan 573` on both arms): the **adoption gate** (no paired CI excluding 0 in the negative direction on MRR/recall@10/20, any of 3 views) and the **drift gate** (control vs the 09-20 pin, within ±0.02 MRR / −0.02 recall@20) both **PASSED**. 63q determinism confirmed bit-identical on both arms (0 non-timing movers).

**Honest reading**: treatment vs control point estimates were MRR-negative on all 3 views (−0.005 to −0.017) with wide CIs spanning 0 on every metric; adoption rests on the gate criteria (statistical indistinguishability) plus the correctness case — the defect is real and the 63/133-query golden set was never positioned to detect the 2.7% of chunks it touches either way. See `evaluation/CANON_20260923_REBASELINE.md` for the full control-vs-treatment table and both gates' CIs.

| Dataset | Queries | MRR | Recall@5 | Recall@10 | Recall@20 | NDCG@5 | pool_hit_rate |
|---|---|---|---|---|---|---|---|
| Canonical (`golden_dataset.json`, A–F excl. D) | 63 | **0.8124** | 0.6448 | 0.7338 | 0.8079 | 0.6679 | 1.0000 |
| Expanded (`golden_dataset_expanded.json`, non-D, 133 queries) | 133 | **0.6431** | 0.6086 | 0.7189 | 0.7843 | 0.5906 | 0.9248 |
| F-via-similar (anchor-chunk view, whole-63q aggregate) | 63 | **0.8696** | 0.6464 | 0.7414 | 0.7880 | 0.6845 | 1.0000 |

Both caveats from the 2026-09-20 section below still apply unchanged: (1) these numbers require the non-default `search_config.json` this machine ships (`codefuse-ai/F2LLM-v2-0.6B` + `jinaai/jina-reranker-v3`); (2) measured with the intent layer pinned off, not the live `default_intent: "HYBRID"` serving config.

### Results (2026-09-20, hybrid-only, k=10, deterministic, resolver-mix restoration re-pin — superseded by the 09-23 pins above)

Provenance: `evaluation/CANON_20260920_REBASELINE.md`, `scripts/benchmark/run_sscg_benchmark.py --project-path .`, default config (`bm25_weight=0.35`, `dense_weight=0.65`, `query_expansion.enabled=False`, `intent.enabled=true` matching the shipped default), PYTHONHASHSEED=0 deterministic harness (ADR-0021), `CLAUDE_AUTO_REINDEX=0` exported for every leg. Re-pin discovered that the 2026-09-08 pin below (and every re-baseline from 09-03 through 09-08) was measured on a **pyan-dark substrate**: the venv had silently drifted 34 packages behind `uv.lock` since 2026-09-02, so `pyan_available()` reported the tier usable while its real `ImportError` fired inside a resolver subprocess and was swallowed as non-fatal (`ee63b8ef`, 2026-09-14). Restoring the venv brings the pyan cross-module call-edge tier back online — 238 files / 3,061 chunks, call graph 7,050 nodes / 31,567 edges (+195 nodes / +957 edges vs the 09-08 pin), resolver mix on `calls` edges `lsp 2029 / chunker 1132 / libcst 775 / pyan 573`. 63q determinism confirmed bit-identical across two independent rounds at clean HEAD (0 movers).

**Two caveats affecting every row below, not unique to this pin:** (1) these numbers require the **non-default** config `search_config.json` ships on this machine — `codefuse-ai/F2LLM-v2-0.6B` + `jinaai/jina-reranker-v3`; the packaged install defaults are `BAAI/bge-m3` + `gte-reranker-modernbert-base` for VRAM reasons (see the 2026-09-14 embedding-model-split bullet below), and a fresh default install should not expect these figures. (2) all numbers are measured with the intent layer pinned **off** (`run_sscg_benchmark.py`'s `pin_intent_off=True`), which is **not** the live serving config — `search_config.json` sets `default_intent: "HYBRID"`, so `GraphScoringStage._reorder_synthetic`'s `module:`-chunk demotion is inert here but active on the running MCP server. Live spot-checks confirm the gap is real: for query Q43, the benchmark's stored top-10 has the gold class chunk at rank 4 with its `module:`-kind sibling occupying a slot ahead of it; a live `search_code` call against the same index (intent on) demotes that `module:` chunk to the tail and the gold moves up. This is a harness-measurement artifact, not something a live user sees.

| Dataset | Queries | MRR | Recall@5 | Recall@10 | Recall@20 | NDCG@5 | pool_hit_rate |
|---|---|---|---|---|---|---|---|
| Canonical (`golden_dataset.json`, A–F excl. D) | 63 | **0.8330** | 0.6390 | 0.7414 | 0.8126 | 0.6669 | 1.0000 |
| Expanded (`golden_dataset_expanded.json`, non-D, 133 queries) | 133 | **0.6514** | 0.6171 | 0.7143 | 0.7684 | 0.5983 | 0.9098 |
| F-via-similar (anchor-chunk view, whole-63q aggregate) | 63 | **0.8876** | 0.6441 | 0.7397 | 0.7897 | 0.6856 | 1.0000 |

Deltas vs the 2026-09-08 pin (0.8241/0.6469/0.8671) are **+0.0089 / +0.0045 / +0.0205** on MRR and **−0.0201 / −0.0214 / −0.0174** on recall@20 — the pre-registered gate (`|ΔMRR| ≤ 0.02`, `Δrecall@20 ≥ −0.02`) is **marginally breached on all three views**, by 0.0001–0.0014. Every breach lands on exactly the metric the resolver-mix restoration would be expected to move (recall@20 pushed down or MRR pushed up by centrality-mediated reordering through the restored pyan tier — see `evaluation/CANON_20260920_REBASELINE.md`'s Part 1 section for the full diagnosis, including the ruled-out gold-set and BM25-corpus-statistics alternatives), `pool_hit_rate` is unchanged or improved on every view, and this pin is adopted as canon-of-record anyway: the alternative is keeping numbers measured on a resolver tier that was silently broken. Reported as a declared breach, not rounded into a pass.

### Results (2026-09-14B, hybrid-only, k=10, deterministic, `BAAI/bge-m3` lineage — RTX 4060 Laptop GPU / 8.6 GB, NOT this workstation, LSP Stage-3 gate)

Provenance: `evaluation/CANON_20260914B_LSP_REBASELINE.md`. This is a **different physical
machine** — a notebook with an RTX 4060 Laptop GPU (8.6 GB VRAM) — running the packaged
`[DEFAULT]` embedder, `BAAI/bge-m3`, a separate, non-comparable lineage from the
`codefuse-ai/F2LLM-v2-0.6B` table above, which is this repo's canon-of-record on the RTX 4090 /
23 GB workstation (see the 2026-09-14 comparability-breaks bullet below).
...
- The 2026-09-08 re-pin (`evaluation/CANON_20260908_REBASELINE.md`, ADR-0069 phantom-edge-shadowing fix): 63q **0.8241**, 133q **0.6469**, F-via-similar **0.8671** on 235 files / 2,978 chunks. Deltas vs 09-06 (+0.0090 / +0.0145 / +0.0014) inside the drift band. Every pin from `evaluation/EMBEDDER_F2LLM_AB_20260726.md` (F2LLM adopted as the deployed model over bge-m3, ≥12 GB VRAM machines) through this one used F2LLM-v2-0.6B, even though the 09-06/09-08 docs stopped restating it. **SUPERSEDED (2026-09-20) — measured on a pyan-dark substrate**: this pin (and every re-baseline from 09-03 through it) was captured before `ee63b8ef` (2026-09-14) fixed a venv drift that had silently zeroed the pyan cross-module call-edge tier since `f5acd585` (2026-09-02); the fix landed on the separate bge-m3 machine's timeline first (line below) and was not applied to this F2LLM machine's venv until the 09-20 re-pin. See `evaluation/CANON_20260908_REBASELINE.md`'s banner and `evaluation/CANON_20260920_REBASELINE.md`.
- **The 2026-09-20 re-pin** (`evaluation/CANON_20260920_REBASELINE.md`) restores the pyan tier on this F2LLM machine (238 files / 3,061 chunks, call graph 7,050 nodes / 31,567 edges, resolver mix `lsp 2029 / chunker 1132 / libcst 775 / pyan 573`). Deltas vs 09-08 (+0.0089 / +0.0045 / +0.0205 MRR, −0.0201 / −0.0214 / −0.0174 recall@20) marginally breach the pre-registered ±0.02 band on all three views (by 0.0001–0.0014) — attributed to the restored pyan tier's centrality-mediated reordering (Part 1 of the canon doc), not a code regression, and adopted anyway per that doc's reasoning. Superseded by the 2026-09-23 re-pin below.
- **The 2026-09-23 re-pin** (`evaluation/CANON_20260923_REBASELINE.md`) adopted `EmbeddingConfig.body_truncation="fill_budget"` after a pre-registered A/B against the 09-20 pin (238 files / 3,071 chunks, resolver mix `lsp 2041 / chunker 1133 / libcst 775 / pyan 573`, identical on both arms) and is the **current canon-of-record**, published as the headline table above. Both gates (adoption: no paired CI excludes 0 negative on MRR/recall@10/20 across all 3 views; drift: control vs 09-20 within ±0.02 MRR / −0.02 recall@20) **PASSED**; treatment point estimates were MRR-negative on all 3 views (−0.005 to −0.017, all CIs spanning 0) — adopted on the gate criteria plus the correctness case, not a quality win.
...
(`rep1/item1` -> `ctrl1`, `td_edge_type: "clone"`; mapped onto `RelationshipType.CLONES` as of
ADR-0072, superseding the `REFERENCES_OP` mapping this measurement was originally taken against)
— and needed one rewording pass: naming the anchor directly (`"...clone instance of the ctrl1 master
