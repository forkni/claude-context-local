...
### Results (2026-08-14, hybrid-only, k=10, deterministic)

Provenance: `evaluation/REMAINING_LEVERS_AB_20260814.md`, `scripts/benchmark/run_sscg_benchmark.py --project-path .`, default config (`bm25_weight=0.35`, `dense_weight=0.65`, `query_expansion.enabled=False`, `--set intent.enabled=true` matching the shipped default), PYTHONHASHSEED=0 deterministic harness (ADR-0021). These are the base runs of the 2026-08-14 remaining-levers campaign, measured on the post-implementation substrate (B1/B4/A4-mechanism/A3-probe commits landed, one incremental reindex, 2,403 vectors). The 63q run was repeated twice and came back **bit-identical (0 per-query movers)** — the deterministic harness eliminates run-to-run flap entirely; residual "noise" now lives only in substrate drift between code changes. Only **hybrid** (the default mode) has been measured at this generation — no per-mode A/B has been rerun since 2026-06-08 (historical table below).
...
Deltas vs the prior `canon_l1` figures (0.8603/0.6789) are substrate drift across the intervening commits (Track A A1/A2, jina-v3.5 support, B1/B4/A4/A3-probe), not attributable to any single change — the 63q r1==r2 identity doubles as proof the implementation commits were quality-neutral at their defaults. The expanded-set hard-miss cohort is substrate-dependent (H034/H066 exited the mrr=0 set this round with no intervention; Q101/Q106/Q117/Q122/H008/H050 remain) — re-derive the miss list from the current baseline before targeting it.
...
- The 2026-08-14 retrieval campaigns (Track A A1/A2, then remaining levers — `evaluation/REMAINING_LEVERS_AB_20260814.md` and `evaluation/GRAPH_RESERVE_PROBE_20260814.md`) re-pinned both canons on the post-Track-A/B1/B4/A4/A3-probe substrate: 63q **0.8722**, 133q **0.6843** — the published baseline above, superseding `canon_l1`'s 0.8603/0.6789. The campaign itself flipped no defaults: A1/A2 not adopted, A4 `signature_head` rejected (CI-negative recall on both sets), A3 reserve not built (probe gate failed), jina-reranker-v3.5 rejected as default; B1 `hide_ambiguous` and B4 `include_top_callers` shipped as display-layer opt-ins with no benchmark surface.
