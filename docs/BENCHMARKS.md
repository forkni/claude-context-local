...
### Results (2026-08-22, hybrid-only, k=10, deterministic, LSP re-baseline)

Provenance: `evaluation/CANON_20260822_LSP_REBASELINE.md`, `scripts/benchmark/run_sscg_benchmark.py --project-path .`, default config (`bm25_weight=0.35`, `dense_weight=0.65`, `query_expansion.enabled=False`, `--set intent.enabled=true` matching the shipped default), PYTHONHASHSEED=0 deterministic harness (ADR-0021). Measured after the `[lsp]` extra — pruned by a bare `uv sync` 2026-08-20, re-locked 2026-08-22 — was confirmed live via a full reindex (217 files/2,611 chunks; resolver mix `lsp 1355 / pyan 1136 / libcst 475`). The 63q run was repeated twice and came back **bit-identical (0 per-query movers)**; with determinism reconfirmed on this substrate, the 133q and F-via-similar views were each captured as a single round. Only **hybrid** (the default mode) has been measured at this generation — no per-mode A/B has been rerun since 2026-06-08 (historical table below).
...
Deltas vs the prior 2026-08-14 pin (0.8722/0.6843) are a **comparability break**: index growth (2,403→2,611 vectors) compounded with the 08-20→08-22 `[lsp]` dark/restore cycle, not attributable to any single change — the 63q r1==r2 identity doubles as proof the measurement itself is stable on this substrate. The expanded-set hard-miss cohort is substrate-dependent and was re-derived this generation (`evaluation/CANON_20260822_LSP_REBASELINE.md`): Q101/Q103/Q106, H004/H008/H021/H033/H034/H050/H054/H063/H066 (12/133, 11 of them H-category) — re-derive the miss list from the current baseline before targeting it.
