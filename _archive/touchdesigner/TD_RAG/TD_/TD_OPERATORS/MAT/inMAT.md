...
The In MAT is used to create a  MAT input in a Component.
...
```python
# Access the MAT inMAT operator
inmat_op = op('inmat1')

# Get/set parameters
freq_value = inmat_op.par.active.eval()
inmat_op.par.active = 1
```
...
inmat_op.inputConnectors[0].connect(input_op)
output_op.inputConnectors[0].connect(inmat_op)
```
