...
|-----------|-------|------|-------------|
| `applyprojmaps` | Apply Projection Maps | Toggle | The projection map texture color from a Light COMP will be applied to the object as well. This allows for applying a projection map without affecting the projection map's color with any lighting. |
...
| `applypointcolor` | Apply Point Color | Toggle | Multiplies the color by the geometry's point color values. |
| `colormap` | Color Map | TOP | Provides a TOP texture to use as a color map. |
...
```python
# Access the MAT constantMAT operator
constantmat_op = op('constantmat1')

# Get/set parameters
freq_value = constantmat_op.par.active.eval()
constantmat_op.par.active = 1
```
...
constantmat_op.inputConnectors[0].connect(input_op)
output_op.inputConnectors[0].connect(constantmat_op)
```
