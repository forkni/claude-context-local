...
The GLSL MAT allows you to write or import custom materials into TouchDesigner.
...
| `vdat` | Vertex Shader | DAT | Path to the DAT that holds the vertex shader code. |
| `pdat` | Pixel Shader | DAT | Path to the DAT that holds the pixel shader code. |
| `loaduniformnames` | Load Uniform Names | Pulse | This will read all the declared (and used) uniforms in the compiled GLSL shader, and fill in the various name fields for the uniform parameters. |
| `clearuniformnames` | Clear Uniform Names | Pulse | This will clear all of the name fields for the uniform parameters. |
| `gdat` | Geometry Shader | DAT | Path to the DAT that holds the geometry shader code. |
| `inherit` | Inherit Uniforms/Samplers from | MAT | This Material will inherit all of the Textures and Uniforms from the GLSL Material referenced in this parameter. |
| `lightingspace` | Lighting Space | Menu | Allows lighting space switch from the current default World Space to legacy Camera Space which was used for TouchDesigner 088. |
...
| `inprim` | Input Primitive Type | Menu | The type of geometry that will be inputed into the Geometry Shader. |
| `outprim` | Output Primitive Type | Menu | The type of geometry that the Geometry Shader will output. |
| `numout` | Num Output Vertices | Int | The maximum number of vertices the Geometry Shader will output. |
| `twocolor` | Two Sided Coloring | Toggle | Enables support for two-sided coloring. When this is enabled the Vertex and/or Geometry shader can write to gl_FrontColor, gl_BackColor, gl_FrontSecondaryColor and gl_BackSecondaryColor and the cor... |
| `attr` | Attribute | Sequence | A sequence of attributes that should be made accesible in the shader. In the past these would be manually declared inside the shader, but to add support for POP based workflows, they need to be set... |
| `sampler` | Sampler | Sequence | Sequence of samplers for use in the shader |
| `vec` | Vector | Sequence | Sequence of vectors to be used as uniforms |
| `array` | Array | Sequence | Sequence of arrays to be used as uniforms |
| `matrix` | Matrix | Sequence | Sequence of matrices to be used as uniforms |
| `rel` | Relative | Sequence | A sequence of relative transforms. The transform is the relative transform one Object COMP to another. |
