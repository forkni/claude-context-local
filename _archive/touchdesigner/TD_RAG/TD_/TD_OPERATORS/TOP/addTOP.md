...
The Add TOP composites the input images together by adding the pixel values.
...
```python
# Access the TOP addTOP operator
addtop_op = op('addtop1')

# Get/set parameters
freq_value = addtop_op.par.active.eval()
addtop_op.par.active = 1
```
...
addtop_op.inputConnectors[0].connect(input_op)
output_op.inputConnectors[0].connect(addtop_op)
```
