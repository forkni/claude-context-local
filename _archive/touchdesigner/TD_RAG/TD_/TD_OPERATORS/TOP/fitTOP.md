...
|-----------|-------|------|-------------|
| `fit` | Fit | Menu | Determines how the input is fit to the specified resolution. |
...
| `t` | Translate | XY | The two fields for Translate and Scale allows you to specify transforms in x and y axes. The field for Rotate allow you to specify the amount of rotation. |
| `tunit` | Translate Unit | Menu | Sets the units used in the Translate parameter. |
| `r` | Rotate | XYZ | The field for rotation allows you to specify the amount of rotation of the image. |
...
| `bgcolor` | Background Color | RGBA | Color applied behind the foreground image. The background is visible when the image is translated or scaled down. Try scaling an image down 50% in size (Scale = 0.5,0.5) and setting the background ... |
