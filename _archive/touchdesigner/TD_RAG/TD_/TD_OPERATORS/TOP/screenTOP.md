...
The Screen TOP brightens the underlying layers depending on how bright the screened layer's pixels are.
...
```python
# Access the TOP screenTOP operator
screentop_op = op('screentop1')

# Get/set parameters
freq_value = screentop_op.par.active.eval()
screentop_op.par.active = 1
```
...
screentop_op.inputConnectors[0].connect(input_op)
output_op.inputConnectors[0].connect(screentop_op)
```
