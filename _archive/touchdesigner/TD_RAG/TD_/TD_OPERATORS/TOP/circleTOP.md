...
|-----------|-------|------|-------------|
| `radius` | Radius | XY | X and Y radii of the Circle. For polygons, only the X radius is used. |
| `radiusunit` | Radius Unit | Menu | Select the units for this parameter from Pixels, Fraction (0-1), Fraction Aspect (0-1 considering aspect ratio). |
...
| `justifyh` | Justify Horizontal | Menu | Specify the horizontal alignment of the circle. |
| `justifyv` | Justify Vertical | Menu | Specify the vertical alignment of the circle. |
...
| `arcangle` | Arc Angles | Float | Specifies the angles at which the shape is to start and end. The region between beginarcangle and endarcangle, with clockwise rotation, will not be drawn. |
...
| `ispolygon` | Polygon | Toggle | When on, a polygon will be drawn. Otherwise a circle is drawn. |
| `sides` | Sides | Int | When drawing a polygon, this corresponds to the number of sides the polygon should have. |
