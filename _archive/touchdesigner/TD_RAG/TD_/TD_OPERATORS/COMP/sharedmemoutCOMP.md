# COMP sharedmemoutCOMP

## Overview

The Shared Mem Out COMP will write transform data to a shared memory block.

## Parameters

| Parameter | Label | Type | Description |
|-----------|-------|------|-------------|
| `active` | Active | Toggle | Controls if this node cooks every frame or not. |
| `name` | Shared Mem Name | Str | The name of the memory block it writes to. |
...
```python
# Access the COMP sharedmemoutCOMP operator
sharedmemoutcomp_op = op('sharedmemoutcomp1')

# Get/set parameters
freq_value = sharedmemoutcomp_op.par.active.eval()
sharedmemoutcomp_op.par.active = 1
```
...
sharedmemoutcomp_op.inputConnectors[0].connect(input_op)
output_op.inputConnectors[0].connect(sharedmemoutcomp_op)
```
