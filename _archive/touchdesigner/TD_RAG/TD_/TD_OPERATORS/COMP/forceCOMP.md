...
=Force COMPs are used to added forces to a physics solver's simulation. Bullet supports linear/rotational forces and impulse forces (see Force page) and Flex supports force fields.

## Parameters

| Parameter | Label | Type | Description |
|-----------|-------|------|-------------|
| `active` | Active | toggle | Toggle the active force on/off in the simulation |
| `force` | Force | float | The linear force in Newtons that will be applied. |
...
| `impulse` | Impulse Force | pulse | Applies an impulse force in the simulation for 1 frame with the above parameters. |
| `ffactive` | Active | toggle | Toggle the force field on/off in the simulation |
| `strength` | Strength | toggle | The strength of the force field. Positive strength pushes bodies outward and negative strength pulls bodies inward. |
| `radius` | Radius | toggle | The radius of the force field. |
| `falloff` | Linear Falloff | toggle | Applies linear falloff to the strength of the force field based on the distance from the center. |
