...
The Text COMP is a panel used to display read-only text or editable text.

## Parameters

| Parameter | Label | Type | Description |
|-----------|-------|------|-------------|
| `mode` | Mode | Menu | Controls whether the contents are taken from the Text parameter or from a Specification DAT/CHOP. |
| `text` | Text | Str | This parameter is the data source for the text that is displayed in the panel. The value may be a number, string, or a reference to an external data source using a python or bind expression. When t... |
| `specdat` | Specification DAT | DAT | A path to a Table DAT that is the source of the text data when in Specification mode. The table should have 3 columns labeled x, y and text in their first rows. Each row after that will display one... |
| `specchop` | Specification CHOP | CHOP | A path to a CHOP that can provide numerical data when in Specification mode. Channels named x and y can be used to position the text data specified in the Specificaction DAT. Each data row in the D... |
| `type` | Type | Menu | The Type parameter controls how the value is interpreted and as well as what formatting and editing options are available. |
| `formatting` | Formatting | Menu | This parameter controls how the source text is is formatted and displayed inside the panel. The options available will change depending on the selected mode. For example, in Float mode there are op... |
| `customformatting` | Custom Formatting | Str | This parameter provides the formatting string for custom modes like Python F-String and C++ Fmt. In both cases, the source text is defined inside curly braces ( {} ) while text outside the braces i... |
| `precision` | Precision | Int | Used in Float mode to set the number of decimal places. |
| `thousandsseparator` | Thousands Separator | Menu | Used in Float and Integer mode to add a separator between thousands to make it easier to read large numbers. For example 1,000,000 |
| `framerate` | Framerate | Int | Sets the number of frames per second when formatting integers into time codes. |
| `smartquote` | Use Smart Punctuation | Toggle | Enables the use of smart punctuation. For example, pairs of quotes will be replaced with appropriate angled quotes, 3 periods will be replaced with an ellipses character and two hypens will become ... |
| `formatcodes` | Formatting Codes | Toggle | Enables the use of formatting directives in the text. For example, the code {#color(255, 0, 0);} will turn all text afterwards on that line red. See Text Formatting Codes for all available codes. |
...
| `wordwrap` | Word Wrap | Toggle | Enables word wrap and can only be used when the 'Type' parameter above is set to 'Multi Line'. Word wrap will split single lines of text onto multiple lines when they extend past the edge of the pa... |
| `editmode` | Edit Mode | Menu | Controls whether the text can be editing directly in the viewer. |
| `shiftenter` | Shift-Enter for New Line | Toggle | In multiline mode, when this option is enabled, pressing Enter will complete editing and save the value and users must hold Shift and Enter if they wish to create a new line. |
| `readingdirection` | Reading Direction | Menu | Use this parameter to control whether the text is displayed from left-to-right (LTR) or from right-to-left (RTL). Bidirectional layout is also supported when LTR characters are detected when using ... |
| `dragdropmode` | Drag Drop Behavior | Menu | This parameter controls what happens when a user drags something onto an active panel. |
| `fieldfocus` | Focus Order | DAT | A path to a Table DAT containing a list of component paths that is used to control which panel receives the focus when a user hits TAB or Shift+Tab during editing. |
| `callbacks` | Callbacks DAT | DAT | The path of a Text DAT containing python callback functions that can be used to react to different events in the viewer. |
| `legacyfontselection` | Legacy Font Selection | Toggle | Enable to select the font using the legacy system of bold and italic toggles. In the new system, bold and italic options can be chosen using the typeface parameter when available. |
| `font` | Font | Menu | Select the font to be used from the dropdown menu. Available fonts are those that have been registered with the operating system. There may be a delay when selecting fonts that have not been used b... |
...
| `typeface` | Typeface | Menu | Select a specific typeface for the current font e.g. Bold, Regular, Italic, etc. Not applicable in legacy font mode. |
| `bold` | Bold | Toggle | Display the text in bold. |
| `italic` | Italic | Toggle | Display the text in italics. |
| `dropshadow` | Drop Shadow | Toggle | Draw a drop shadow under the text. |
| `scaletofit` | Scale Text to Fit | Menu | Automatically adjust the font size to fit inside the panel. |
| `fontsize` | Font Size | Float | Set the size of the font. By default the font is defined in resolution-independent panel units that will be rendered to a variable number of pixels depending on DPI settings of the output window (e... |
| `fontsizeunits` | Font Size Units | Menu | Determines how the font size is interpretted. The default is resolution-independent panel units. |
