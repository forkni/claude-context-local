...
|-----------|-------|------|-------------|
| `callbacks` | Callbacks | DAT | A reference to the callback DAT. |
| `rows` | Rows | Int | The number of rows in the List. |
| `cols` | Columns | Int | The number of columns in the List. |
| `lockfirstrow` | Lock First Row | Toggle | Locks the first row in place when scrolling horizontally. |
| `lockfirstcol` | Lock First Col | Toggle | Locks the first column in place when scrolling vertically. |
| `hscrollbar` | Horizontal Scroll Bar | Toggle | Enables the horizontal scroll bar. |
| `vscrollbar` | Vertical Scroll Bar | Toggle | Enables the vertical scroll bar. |
| `offcellcallbacks` | Off Cell Callbacks | Toggle | When on, you get callbacks (select, rollover, etc) on List COMP areas that are not in any cells. For example, the area below a list when the list has a large size but only a few entries at the top. |
| `reset` | Reset | Pulse | Resets the table by running its initialize callbacks. |
