...
| `interactive` | Interactive | Toggle | Enable or disable mouse or touch screen interaction with the target operator's viewer. |
| `opcenter` | Center | XY | Lets you pan across the target operator's viewer when the Scale is > 1. Center adjusts what part of the target operator's viewer is in the middle of the OP Viewer. |
| `opcenterunit` | Center Unit | Menu | Center can be expressed in panel units or fraction of width, or display points. |
| `opscale` | Scale | Float | Lets you zoom in on parts of the target operator's viewer. Increasing scale magnifies the viewer. |
| `topdirect` | Draw TOP Directly | Toggle | When this is On for TOPs, you always see the RGBA image, as if this was simply a Select TOP. Normally you see what the viewer is currently displaying, which for TOPs may have overlays and not be di... |
| `contentsize` | Use Content Size | Toggle | Only applicable for viewing TOPs, when this option is On, the Center position is calculated based on the dimensions of the texture rather than the panel i.e. panel units are treated as pixels when ... |
...
```python
# Access the COMP opviewerCOMP operator
opviewercomp_op = op('opviewercomp1')

# Get/set parameters
freq_value = opviewercomp_op.par.active.eval()
opviewercomp_op.par.active = 1
```
...
opviewercomp_op.inputConnectors[0].connect(input_op)
output_op.inputConnectors[0].connect(opviewercomp_op)
```
...
This operator has **115** documented parameters.
