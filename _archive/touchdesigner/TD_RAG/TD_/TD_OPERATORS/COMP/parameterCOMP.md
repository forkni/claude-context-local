...
The Parameter Component allows any operator's parameter dialog to be a panel with full interactivity.

## Parameters

| Parameter | Label | Type | Description |
|-----------|-------|------|-------------|
| `op` | Operator | OP | Path to the operator whose parameters will be used. |
| `header` | Header | Toggle | Include the header of the parameter dialog. This is the section with OP type, name, and help buttons. |
| `pagenames` | Page Names | Toggle | Include the tabs for the parameter pages in the display. |
| `separators` | Separators | Toggle | Display the separating lines between sections of parameters. |
| `inputeditor` | Input Editor | Toggle | Include the multi-input editor UI.  This appears at the end of multi-input parameter dialogs such as the one used in the Math CHOP or Merge SOP.           Image:InputEditor.png |
| `allowexpend` | Allow Expansion | Toggle | Controls whether or not parameters can be expanded to reveal parameter modes and expression fields. |
| `builtin` | Built-In | Toggle | Include the operator's built-in parameters. |
| `custom` | Custom | Toggle | Include the operator's Custom Parameters. |
| `combinescopes` | Combine Scopes | Menu | Specify the how to combine the scope parameters below to make the parameter selection. |
| `pagescope` | Page Scope | StrMenu | Selects with pages of parameters to include. Use * for Pattern Matching all. |
| `parscope` | Parameter Scope | StrMenu | Selects which parameters to include. Use * for Pattern Matching all. |
| `scopeorder` | Order by Scope | Toggle | Displays the parameters in the order they are defined in the Parameter Scope parameter above. Only available when Page Name parameter is Off. |
| `compress` | Compress | Float | When you change the Compress value, the panel stays exactly the same width and height as specified by the Width and Height parameters, but it renders more or less of the parameter dialog in it. Hen... |
...
```python
# Access the COMP parameterCOMP operator
parametercomp_op = op('parametercomp1')

# Get/set parameters
freq_value = parametercomp_op.par.active.eval()
parametercomp_op.par.active = 1
```
...
parametercomp_op.inputConnectors[0].connect(input_op)
output_op.inputConnectors[0].connect(parametercomp_op)
```
...
This operator has **122** documented parameters.
