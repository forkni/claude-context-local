...
The Null Component serves as a place-holder in a scene.
...
```python
# Access the COMP nullCOMP operator
nullcomp_op = op('nullcomp1')

# Get/set parameters
freq_value = nullcomp_op.par.active.eval()
nullcomp_op.par.active = 1
```
...
nullcomp_op.inputConnectors[0].connect(input_op)
output_op.inputConnectors[0].connect(nullcomp_op)
```
...
This operator has **54** documented parameters.
