...
|-----------|-------|------|-------------|
| `label` | Label | Str | The label that appears on the button |
| `value0` | Value | Toggle | The button's state |
| `buttontype` | Button Type | Menu | This menu determines the button's state behavior. |
| `buttongroup` | Button Group Label | Str | Create a radio button group using this parameter. Buttons grouped using this parameter must all be located in the same component. Any buttons with the same group label will be in the same radio group. |
| `buttongroupdat` | Button Group DAT | DAT | Specify a Table DAT that lists all buttons in a radio group. This allows a group of radio buttons to have its member buttons located in any network - the buttons do not have to be in the same compo... |
| `color` | Color | RGB | RGB tone for the button elements |
...
```python
# Access the COMP buttonCOMP operator
buttoncomp_op = op('buttoncomp1')

# Get/set parameters
freq_value = buttoncomp_op.par.active.eval()
buttoncomp_op.par.active = 1
```
...
buttoncomp_op.inputConnectors[0].connect(input_op)
output_op.inputConnectors[0].connect(buttoncomp_op)
```
