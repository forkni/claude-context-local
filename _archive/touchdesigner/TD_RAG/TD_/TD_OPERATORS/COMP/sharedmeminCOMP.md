# COMP sharedmeminCOMP

## Overview

The Shared Mem In COMP will read transform data from a shared memory block.
...
```python
# Access the COMP sharedmeminCOMP operator
sharedmemincomp_op = op('sharedmemincomp1')

# Get/set parameters
freq_value = sharedmemincomp_op.par.active.eval()
sharedmemincomp_op.par.active = 1
```
...
sharedmemincomp_op.inputConnectors[0].connect(input_op)
output_op.inputConnectors[0].connect(sharedmemincomp_op)
```
