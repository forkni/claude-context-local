...
|-----------|-------|------|-------------|
| `label` | Label | Str | The label that appears on the slider |
| `slidertype` | Slider Type | Menu | Sets the type of slider. |
| `value0` | Value | Float | The slider's position for one dimensional sliders (U or V) |
| `value1` | Value 1 | Float | The slider's V position for two dimensional sliders (UV) |
| `zonel` | Zone Left | Float | Sets the left bounding area that the slider will track movement in. 0 is the left edge of the panel, 1 is the right edge of the panel. Values outside the 0-1 range can be used. |
| `zoner` | Zone Right | Float | Sets the right bounding area that the slider will track movement in. |
| `zoneb` | Zone Bottom | Float | Sets the bottom bounding area that the slider will track movement in. |
| `zonet` | Zone Top | Float | Sets the top bounding area that the slider will track movement in. |
| `clampul` | Clamp U Low | Toggle | Clamps the minimum output of the panel value u to 0. |
| `clampuh` | Clamp U High | Toggle | Clamps the maximum output of the panel value u to 0. |
| `clampvl` | Clamp V Low | Toggle | Clamps the minimum output of the panel value v to 0. |
| `clampvh` | Clamp V High | Toggle | Clamps the maximum output of the panel value v to 0. |
...
```python
# Access the COMP sliderCOMP operator
slidercomp_op = op('slidercomp1')

# Get/set parameters
freq_value = slidercomp_op.par.active.eval()
slidercomp_op.par.active = 1
```
...
slidercomp_op.inputConnectors[0].connect(input_op)
output_op.inputConnectors[0].connect(slidercomp_op)
```
