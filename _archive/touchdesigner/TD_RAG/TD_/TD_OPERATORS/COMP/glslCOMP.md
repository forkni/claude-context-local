...
The GLSL COMP uses shaders to render an image in a panel directly to the screen.

## Parameters

| Parameter | Label | Type | Description |
|-----------|-------|------|-------------|
| `vertexdat` | Vertex Shader | DAT | Path to the DAT that holds the vertex shader code. |
| `pixeldat` | Pixel Shader | DAT | Path to the DAT that holds the pixel shader code. |
| `sampler` | Sampler | Sequence | Sequence of TOP samplers |
| `sampler0name` | Name | Str | This is the sampler name that the GLSL program will use to sample from this TOP. The samplers need to be declared at the same dimensions as the TOP (sampler2D for a 2D TOP, sampler3D for 3D TOP). |
| `sampler0top` | TOP | TOP | This is the TOP that will be referenced by the above sampler name above it.      Exposed by the + Button, texture sampling parameters:          Refer to the Texture Sampling Parameters article for ... |
| `vec` | Vector | Sequence | Sequence of named vectors to be used as uniforms in the shader |
| `vec0name` | Name | Str | The name of the uniform. |
| `vec0value` | Value | XYZW | The value to assign to the uniform. If the uniform is a float the first entry of the four is used, if the uniform is a vec2 the first two entries are used, etc. |
| `const0name` | Constant Name | Str | The constant name, as declared in the shader. |
| `const0value` | Value | Float | The value to give the constant. |
...
```python
# Access the COMP glslCOMP operator
glslcomp_op = op('glslcomp1')

# Get/set parameters
freq_value = glslcomp_op.par.active.eval()
glslcomp_op.par.active = 1
```
...
glslcomp_op.inputConnectors[0].connect(input_op)
output_op.inputConnectors[0].connect(glslcomp_op)
```
...
This operator has **119** documented parameters.
