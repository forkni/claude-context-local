...
The Select Component selects a Panel Component from any other location.

## Parameters

| Parameter | Label | Type | Description |
|-----------|-------|------|-------------|
| `selectpanel` | Select Panel | PanelCOMP | The Panel Component which is being selected. |
| `matchsize` | Match Size | Toggle | Set the Select COMP's width and Height to the referenced Panel Component's dimenions. |
| `followselection` | Follow Selection | Toggle | In the case of referencing a Select COMP, Follow Selection will retrieve the dimensions from the upper most component that is not a Select COMP. |
