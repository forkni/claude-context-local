...
def _update_stored_path_if_changed(project_dir: Path, current_path: str) -> None:
...
def get_project_storage_dir(
    ...
    dimension = cast(int, model_config.get("truncate_dim") or model_config["dimension"])
    model_slug = get_model_slug(model_name)

    # Check for existing project directory (handles drive letter changes)
    projects_dir = base_dir / "projects"
    existing_dir = _find_existing_project_dir(
        projects_dir,
        project_name,
        new_hash,
        legacy_hash,
    ...
    if existing_dir:
        # Update stored path if drive letter changed
        _update_stored_path_if_changed(existing_dir, str(project_path))
        logger.info(
