        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "output_format": {
                    ...
                    "default": "compact",
                    "description": "Output format: 'verbose' (full), 'compact' (omit empty, default), 'ultra' (tabular: 'key[N]{field1,field2}': [[val1,val2], ...]). See docs/MCP_TOOLS_REFERENCE.md for details. (schema default: 'compact'; the running server may be configured with a different effective default — check get_search_config_status.output_format)",
                },
                "ego_graph_enabled": {
                    "type": "boolean",
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "include_dirs": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": 'Composition depends on what each pattern points at. A pattern that reaches into a dependency tree (venv, site-packages, node_modules, ...) is ADDITIVE — it is added on top of the normal root-down scope, re-admitting that one path even though its parent is ignored by default (e.g. ["venv/Lib/site-packages/torch"] indexes torch AND your own source, since venv/site-packages are ignored by default but torch is explicitly re-admitted). Any other pattern is NARROWING — it restricts indexing to just the paths named, discarding the rest of the project (e.g. ["src/core", "lib/utils"] indexes ONLY those two directories). Mixing both kinds unions them: narrowing patterns set the base scope, additive patterns add dependency paths on top. Set include_exclusive=true to force every pattern to be narrowing (whitelist-only), e.g. to index ONLY a set of libraries with no first-party source at all. Gitignore-style matching: a pattern with no "/" matches at ANY depth by basename; a pattern containing "/" is root-anchored; wildcards ("*", "?", "[abc]", "**") are supported. Absolute paths are accepted and resolved against the project root. A pattern matching zero files/directories is reported as a warning, not silently dropped; if EVERY include pattern matches nothing, indexing aborts with an error instead of writing an empty index. Omit to reuse the stored value from project creation; pass a new list to replace it wholesale (not merged), or [] to clear it. Changing this on an existing project forces a full (non-incremental) reindex.',
                },
                "exclude_dirs": {
                    "type": "array",
                    "items": {"type": "string"},
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "output_format": {
                    ...
                    "enum": ["verbose", "compact", "ultra"],
                    "default": "compact",
                    "description": "Output format: 'verbose' (full), 'compact' (omit empty, default), 'ultra' (tabular). (schema default: 'compact'; the running server may be configured with a different effective default — check get_search_config_status.output_format)",
                },
            },
            "required": [],
        },
